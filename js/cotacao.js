...
function podeGerenciarCotacao(cotacao) {
  return temCargo(_dadosUsuario, "admin") ||
    (!!_usuario?.uid && cotacao?.criadoPor === _usuario.uid);
}
...
function linhaCotacaoHtml(c) {
  ...
  const podeGerenciar = podeGerenciarCotacao(c);
  return `
    <tr>
      ...
      <td class="td-actions-col col-center">
        <div class="td-actions-wrap td-actions-wrap--cotacoes">
          ...
          ${podeGerenciar ? `<button class="btn-action btn-action--edit"
            data-action="editar" data-id="${escHtml(c.id)}">
            <svg viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
            Editar
          </button>` : ""}
          ${podeGerenciar ? `<button class="btn-action btn-action--delete"
