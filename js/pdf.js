...
export function gerarPDF(cotacao) {
  try {
    ...
    // ── Logo esquerda ──────────────────────────────────────
    const LX = MX, LY = 7, LW = 26, LH = 26;
    // sombra leve
    fill([0,0,0]);
    doc.setGState(doc.GState({ opacity: 0.15 }));
    doc.roundedRect(LX+1, LY+1, LW, LH, 4, 4, "F");
    doc.setGState(doc.GState({ opacity: 1 }));
    // caixa branca
    fill(C.branco);
    ...
    fill([210, 225, 255]);
    doc.roundedRect(LX+2, LY+2, LW-4, LH-4, 2.5, 2.5, "F");
    // letra F
    doc.setFont("helvetica","bold");
    doc.setFontSize(16);
    rgb(C.azulEscuro);
    doc.text("F", LX + LW/2, LY + LH/2 + 3, { align:"center" });
    ...
    doc.text("PAPELARIA", TX, LY + 6);

    doc.setFont("helvetica","bold");
    doc.setFontSize(19);
    rgb(C.branco);
    doc.text("Futura", TX, LY + 16.5);

    doc.setFont("helvetica","normal");
    doc.setFontSize(6.5);
    doc.setTextColor(160, 195, 255);
    doc.text("C E N T R O", TX, LY + 22);
